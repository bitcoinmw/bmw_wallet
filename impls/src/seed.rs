...
use bmw_wallet_util::grin_keychain::{mnemonic, Keychain};
use bmw_wallet_util::grin_util::ToHex;
...
impl WalletSeed {
	...
	pub fn recover_from_phrase(
		...
		if !Path::new(&data_file_dir).exists() {
			return Err(ErrorKind::WalletDoesntExist(
				...
				"To create a new wallet from a recovery phrase, use 'bmw-wallet init -r'"
					.to_owned(),
			)
			.into());
		}
	...
	pub fn from_file(
		...
		if Path::new(seed_file_path).exists() {
			...
			error!(
				"wallet seed file {} could not be opened (bmw-wallet init). \
				 Run \"bmw-wallet init\" to initialize a new wallet.",
				seed_file_path
			);
	...
	pub fn _delete_seed_file(data_file_dir: &str) -> Result<(), Error> {
		let seed_file_path = &format!("{}{}{}", data_file_dir, MAIN_SEPARATOR, SEED_FILE,);
		if Path::new(seed_file_path).exists() {
